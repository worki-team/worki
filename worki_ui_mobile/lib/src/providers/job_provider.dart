...
class JobsProvider {
 ...
 Future<List<Job>> getJobsByCompany(String companyId) async {
  ...
  Future<List<Job>> getJobsByCoordinatorId(String coordinatorId) async {
    final url = Uri.https( _url, '/api/job/coordinator/$coordinatorId');
    return await _procesarRespuesta(url);
  }
  ...
  Future<List<Worker>>getRegisteredWorkersByJobId(String jobId) async {
    final url = Uri.https( _url, '/api/job/$jobId/registered');
    final resp            = await http.get( url );
    final decodedData     = json.decode( utf8.decode(resp.bodyBytes) );
    final workers         = new Workers.fromJsonList(decodedData);
    return workers.items;
 ...
 Future<dynamic> updateJob(Job job) async {
  ...
  Future<List<Job>> getFilteredJobs(Map<String,dynamic> attributes) async{
    ...
    if(attributes['endSalary'] != '999999'){
