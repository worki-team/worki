...
class CoordinatorProvider {
  ...
  Future<Map<String, dynamic>> updateCoordinator(
    ...
    Map<String, dynamic> response = json.decode(utf8.decode(req.bodyBytes));
    await firestoreProvider.updateUser(coordinator.id, coordinator.name,coordinator.profilePic, coordinator.roles[0]);
  ...
  Future<List<Coordinator>> getCoordinatorsByCompanyId(String companyId) async {
    final String coordinatorUrl = '$url/api/coordinator/companyId/$companyId'; //url
    print("Get : " + coordinatorUrl); //print method and url

    final req = await http.get(coordinatorUrl);

    final response = json.decode(utf8.decode(req.bodyBytes));
    final coordinators = new Coordinators.fromJsonList(response);
    return coordinators.items;
    
  }

  Future<void> deleteCoordinator(String coordinatorId) async {
    final String coordinatorUrl = '$url/api/coordinator/$coordinatorId'; //url
    print("Delete : " + coordinatorUrl); //print method and url

    final req = await http.delete(coordinatorUrl);
  }
}
