    ...
    firebaseProvider.checkStatus().then((FirebaseUser user) async{
      if(user == null){
        ...
        if(userResult['roles'][0] == 'COORDINATOR'){
          ...
          Company company = await _companyProvider.getCompanyById(coordinator.companyId);
          Navigator.of(context).pushReplacementNamed('coordinator_main', arguments: {'coordinator':coordinator,'company':company});
        }
      }
    });
  }
