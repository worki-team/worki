...
class WorkerProvider {
  ...
  Future<Map<String, dynamic>> updateWorker(Worker worker) async {
    ...
    print('USUARIO::' + worker.toJson().toString());
    print('CIUDADRESIDENCIA::' + worker.toJson().toString());
    ...
    Map<String, dynamic> response = json.decode(utf8.decode(req.bodyBytes));
    final fireAnswer = await firestoreProvider.updateUser(
        worker.id, worker.name, worker.profilePic, worker.roles[0]);
    if (response.containsKey('id')) {
      return {'ok': true, 'worker': response};
    } else {
      return {'ok': false, 'message': response['message']};
  ...
  Future<List<Worker>> getFilteredWorkers(Map<String,dynamic> attributes) async{
    ...
    if(attributes['contexture'] != ''){
      _params['contexture'] = attributes['contexture'];
    }
    if(attributes['eyeColor'] != ''){
      _params['eyeColor'] = attributes['eyeColor'];
    }
    if(attributes['hairType'] != ''){
      _params['hairType'] = attributes['hairType'];
    }
    if(attributes['hairColor'] != ''){
      _params['hairColor'] = attributes['hairColor'];
    }
    if(attributes['skinColor'] != ''){
      _params['skinColor'] = attributes['skinColor'];
    }
